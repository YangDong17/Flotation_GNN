...
class LmdbDataset(Dataset):
    ...
    def __init__(self, config, transform=None):
        ...
        if not self.path.is_file():
            ...
            for db_path in db_paths:
                self.envs.append(self.connect_db(lmdb_path=db_path))
                #print(db_path)
                #print(self.connect_db(db_path))
                #print(self.envs)
                #print(self.envs[-1])
                #print(self.envs[-1].begin())
                #print(self.envs[-1].begin().get("length".encode("utf-8")))
                #print(self.envs[-1].begin().get("length".encode("ascii")))
                #print(self.envs[-1].begin().get("0".encode("ascii")))
                #print(self.envs[-1].begin().get("1".encode("ascii")))
                #print(self.envs[-1].begin().get("idx".encode("ascii")))
                length = pickle.loads(
    ...
    def __getitem__(self, idx):
        if not self.path.is_file():
            ...
            self.envs = self.connect_db(db_paths[idx])
            ...
            #print("********************************")
            #print(self._keys[0])
            #print(self.env.begin())
            #print(pickle.loads(self.env.begin().get(self._keys[idx])))
            datapoint_pickled = self.env.begin().get(self._keys[idx])
            #print(datapoint_pickled)
            #print('type:',type(datapoint_pickled))
            #print(pickle.loads(datapoint_pickled))
            data_object = pyg2_data_transform(pickle.loads(datapoint_pickled))
